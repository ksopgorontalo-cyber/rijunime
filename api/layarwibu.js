...
router.get('/episode/:slug', async (req, res) => {
  try {
    ...
    Object.keys(streamingData.mirror).forEach(quality => {
      ...
      streamingData.mirror[quality].forEach(mirror => {
        ...
        if (isDesuplay) {
          ...
          desustreamServers.push(server);
        } else {
          videoServers.push(server);
        }
      });
    });
    ...
    if (desuplayServers.length > 0) {
      // Jika ada desuplay, desuplay di urutan pertama
      sortedVideoServers = [...desuplayServers, ...desustreamServers, ...videoServers];
    } else {
    ...
    const episode = {
      ...
      videoServers: sortedVideoServers,
      downloadLinks,
      prevEpisode,
      nextEpisode,
      animeSlug
